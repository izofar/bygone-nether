...
import com.izofar.bygonenether.entity.ai.behaviour.*;
...
public class PiglinPrisonerAi {
    ...
    private static void initCelebrateActivity(Brain<PiglinPrisoner> brain) {
        brain.addActivityAndRemoveMemoryWhenStopped(Activity.CELEBRATE, 10, ImmutableList.of(
                        ...
                        new RunOne<>(ImmutableList.of(
                                Pair.of(SetEntityLookTarget.create(ModEntityTypes.PIGLIN_PRISONER, 8.0F), 1),
    ...
    private static SoundEvent getSoundForActivity(PiglinPrisoner piglinPrisoner, Activity activity) {
        ...
        } else if (isNearRepellent(piglinPrisoner)) {
            return  SoundEvents.PIGLIN_RETREAT;
        } else {
    ...
    public static void startDancing(AbstractPiglin abstractPiglin) {
        abstractPiglin.getBrain().setMemoryWithExpiry(MemoryModuleType.DANCING, true, CELEBRATION_TIME);
        abstractPiglin.getBrain().setMemoryWithExpiry(MemoryModuleType.CELEBRATE_LOCATION, abstractPiglin.blockPosition(), CELEBRATION_TIME);
    }
    ...
    private static boolean hasCrossbow(LivingEntity entity) {
        return entity.isHolding(is -> is.getItem() instanceof CrossbowItem);
    }

    protected static void broadcastAngerTarget(AbstractPiglin abstractPiglin, LivingEntity target) {
        getAdultPiglins(abstractPiglin).forEach((adultPiglin) -> setAngerTargetIfCloserThanCurrent(adultPiglin, target));
    }
    ...
    private static List<Piglin> getAdultPiglins(AbstractPiglin abstractPiglin) {
        return getAdultAbstractPiglins(abstractPiglin).stream().filter(Piglin.class::isInstance).map(Piglin.class::cast).toList();
    }
